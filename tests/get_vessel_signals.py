...
def main(
    ...
    for v in v_list:
        ...
        if all_signals:
            ...
            int_signals = ["TEMPERATURE", "TURBIDITY", "SALINITY", "OXYGEN", "gpstrack"]
            int_ts = [
                ts
                for sn in int_signals
                for ts in signals
        ...
        if len(int_ts) > 0:
            if noffill:
                data = TimeSeries.get_timeseries_list(
                    tsb_host,
                    int_ts,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        dest="start",
        help="Start time of query interval",
        default=None,
    )
    parser.add_argument(
        "-e",
        ...
        required=False,
        dest="end",
        help="End time of query interval",
        default=None,
    )
    parser.add_argument(
        "-t",
        ...
        required=False,
        dest="token",
        help="JWT token file (JSON format) for NIVA account",
        default=None,
    )
    parser.add_argument(
        "-o",
        ...
        help="Prefix for output files, if not included nothing is stored",
        default=None,
    )
    parser.add_argument(
        "--all-signals",
    ...
    parser.add_argument(
        ...
        required=False,
        dest="dt",
        help="Time window in aggregate, if not included raw data is returned",
        default=None,
    )
    parser.add_argument(
        "--noffill",
