...
def populate_database():
    ...
    unique_paths = list(
        {r[1] for r in test_data["time_series"]}
        .union({r[1] for r in test_data["locations"]})
        .union({r[1] for r in test_data["flags"]})
    )

    response = requests.post(
        urljoin(META_URL, "get_thing_from_path"), json={"paths": unique_paths}
    )
    ...
    test_data = {
        k: [[path_uuid_map[e] if i == 1 else e for i, e in enumerate(r)] for r in v]
...
def test_tsb_for_newly_populated_data(populate_database):
    ...
    new_data = pyniva.get_newly_inserted_data(
        start, end, aggregate=True, meta_host=META_URL, ts_host=TSB_URL
    )
    assert len(new_data) == 26  # 26 paths have new data
    assert all(
        list(key in new_data[0] for key in ["count", "max", "min", "uuid", "path"])
    )

    new_data = pyniva.get_newly_inserted_data(
        start, end, aggregate=False, meta_host=META_URL, ts_host=TSB_URL
    )
    assert len(new_data) == 1326  # 1326 new pieces of data in total
    assert all(
        key in new_data[0] for key in ["created_timestamp", "time", "uuid", "path"]
    )
